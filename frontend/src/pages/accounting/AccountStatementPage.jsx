...
import { StatementMetricCard, StatementSplitCard, statementSurfaceClass } from "../../components/accounting/StatementCards";
...
export default function AccountStatementPage() {
  ...
  const currentMonthRow = useMemo(() => {
    if (!yearStatements.length) return null;

    const now = new Date();
    const currentYear = String(now.getFullYear());
    const currentMonth = String(now.getMonth() + 1).padStart(2, "0");
    const preferredPeriod = `${year}-${currentMonth}`;
    const preferred = year === currentYear
      ? yearStatements.find((item) => String(item.period).slice(0, 7) === preferredPeriod)
      : null;

    const statement = preferred ?? [...yearStatements].sort((first, second) =>
      String(second.period).localeCompare(String(first.period))
    )[0];

    if (!statement) return null;

    const period = String(statement.period).slice(0, 7);
    const month = period.slice(5, 7);
    const label = months.find(([value]) => value === month)?.[1] ?? month;
    return { month, label, period, statement };
  }, [year, yearStatements]);
  ...
  if (!account) return <div className="rounded-lg border border-gray-300 bg-white p-8"><h1 className="text-xl font-semibold">Conto non trovato</h1><Link to="/accounting/statements" className="mt-4 inline-block text-sm text-amber-700">Torna ai conti</Link></div>;
  ...
  return (
    <div className={statementSurfaceClass()}>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <FileText className="h-5 w-5 shrink-0 text-black" />
              <h1 className="text-xl text-black">Estratti conto {account.name} anno {year}</h1>
            </div>
            <p className="mt-1 text-sm text-black">Gestisci i saldi mensili, visualizza le variazioni e i totali annuali.</p>
          </div>

          {currentMonthRow && (
            <div className="relative lg:ml-auto">
              <div className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-slate-900">
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-600">MODIFICA RAPIDA {currentMonthRow.label.toUpperCase()} {year}</p>
                  <p className={`text-sm font-semibold leading-tight ${Number(currentMonthRow.statement.currentBalance) < 0 ? "text-slate-900" : "text-slate-700"}`}>
                    <Amount>{currency.format(Number(currentMonthRow.statement.currentBalance))}</Amount>
                  </p>
                </div>
              ...
              {stickyEdit !== null && (
                <div className="absolute right-0 top-full z-20 mt-2 w-[340px] rounded-lg border border-gray-300 bg-white p-3 text-slate-900 backdrop-blur-sm">
                  <div className="space-y-2">
                    <input
                      ...
                      className="h-8 w-full rounded-md border border-gray-300 bg-white/95 px-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400"
                    />
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="flex-1 rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50"
                      >
                        Annulla
                      </button>
                    </div>
      ...
      {fields && (
        <form ref={formRef} onSubmit={save} className="mt-4 rounded-xl border border-gray-300 bg-white p-5">
            ...
            <FormField label={previousBalanceLabel(fields.period)}>
              {fields.period === "2024-01" ? (
                <input
                  ...
                  className="h-10 w-full rounded-md border border-gray-300 px-3 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400"
                />
              ) : (
                <div className="flex h-10 items-center rounded-md border border-gray-300 bg-slate-50 px-3 text-sm tabular-nums text-slate-500">
                  <Amount>{currency.format(Number(previousValue))}</Amount>
                </div>
              )}
            </FormField>
            <FormField label="Saldo mese corrente *"><input type="number" step="0.01" required value={fields.currentBalance} onChange={(event) => setFields((current) => ({ ...current, currentBalance: event.target.value }))} className="h-10 w-full rounded-md border border-gray-300 px-3 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></FormField>
            <FormField label="Note"><input value={fields.notes} onChange={(event) => setFields((current) => ({ ...current, notes: event.target.value }))} className="h-10 w-full rounded-md border border-gray-300 px-3 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></FormField>
          </div>
          ...
          <div className="mt-4 flex justify-end"><button type="submit" disabled={saving} className="h-9 rounded-md bg-gradient-to-r from-indigo-600 to-indigo-500 px-5 text-sm font-semibold text-white hover:from-indigo-700 hover:to-indigo-600 disabled:opacity-60">{saving ? "Salvataggio..." : "Salva mese"}</button></div>
        </form>
      )}
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatementMetricCard
          ...
          rawValue={metrics.globalLatestBalance}
          icon={BarChart}
          tone="globalBalance"
          size="regular"
        />
      ...
      <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <StatementMetricCard
          label="Miglior estratto conto"
          value={metrics.bestCurrent ? formatCurrencyAmount(metrics.bestCurrent.value, currency) : ""}
          rawValue={metrics.bestCurrent ? metrics.bestCurrent.value : null}
          detail={hasBestCurrent ? monthLabel(metrics.bestCurrent.period) : "Nessun mese compilato"}
          icon={TrendingUp}
          tone={hasBestCurrent && metrics.bestCurrent.value < 0 ? "danger" : "best"}
          size="compact"
          danger={hasBestCurrent && metrics.bestCurrent.value < 0}
        />
        <StatementMetricCard
          label="Peggior estratto conto"
          value={metrics.worstCurrent ? formatCurrencyAmount(metrics.worstCurrent.value, currency) : ""}
          rawValue={metrics.worstCurrent ? metrics.worstCurrent.value : null}
          detail={hasWorstCurrent ? monthLabel(metrics.worstCurrent.period) : "Nessun mese compilato"}
          icon={AlertTriangle}
          tone={hasWorstCurrent && metrics.worstCurrent.value < 0 ? "danger" : "worst"}
          size="compact"
          danger={hasWorstCurrent && metrics.worstCurrent.value < 0}
        />
        <StatementMetricCard
          label="Migliore ricavo"
          value={metrics.bestRevenue ? formatCurrencyAmount(metrics.bestRevenue.value, currency, { signed: true }) : ""}
          rawValue={metrics.bestRevenue ? metrics.bestRevenue.value : null}
          detail={hasBestRevenue ? monthLabel(metrics.bestRevenue.period) : "Nessun mese compilato"}
          icon={TrendingUp}
          tone={hasBestRevenue && metrics.bestRevenue.value < 0 ? "danger" : "best"}
          size="compact"
          danger={hasBestRevenue && metrics.bestRevenue.value < 0}
        />
        <StatementMetricCard
          label="Peggiore ricavo"
          value={metrics.worstRevenue ? formatCurrencyAmount(metrics.worstRevenue.value, currency, { signed: true }) : ""}
          rawValue={metrics.worstRevenue ? metrics.worstRevenue.value : null}
          detail={hasWorstRevenue ? monthLabel(metrics.worstRevenue.period) : "Nessun mese compilato"}
          icon={AlertTriangle}
          tone={hasWorstRevenue && metrics.worstRevenue.value < 0 ? "danger" : "worst"}
          size="compact"
          danger={hasWorstRevenue && metrics.worstRevenue.value < 0}
        />
      </div>

      <TrendChart rows={rows} color={color} />

      <div className={`mt-6 ${statementSurfaceClass()}`}>
        <div className="grid grid-cols-[110px_1fr_1fr_1fr_100px] gap-4 border-b border-gray-300 bg-slate-50 px-5 py-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400"><span>Mese</span><span className="text-right">Estratto Conto Mese Precedente</span><span className="text-right">Estratto Conto Mese Corrente</span><span className="text-right">Ricavo del Mese</span><span /></div>
            ...
            return (
              <div id={`statement-month-${row.month}`} key={row.month} className="grid grid-cols-[110px_1fr_1fr_1fr_100px] items-center gap-4 px-5 py-2.5 text-[13px] transition-colors">
                ...
                <span className="text-right tabular-nums text-slate-500">
                  {previous === undefined ? "—" : <Amount>{currency.format(Number(previous))}</Amount>}
                </span>
                <strong className={`text-right tabular-nums ${statement ? "text-slate-900" : "text-slate-300"}`}>
                  {statement ? <Amount>{currency.format(Number(statement.currentBalance))}</Amount> : "—"}
                </strong>
                <strong className={`text-right tabular-nums ${change === null ? "text-slate-300" : change < 0 ? "text-slate-900" : "text-emerald-600"}`}>
                  {change === null ? "—" : <Amount>{`${change >= 0 ? "+" : ""}${currency.format(change)}`}</Amount>}
                </strong>
                <div className="flex justify-end gap-1">{statement ? <><button type="button" onClick={() => openRow(row)} aria-label={`Modifica ${row.label}`} className="flex h-7 w-7 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-700"><SquarePen className="h-4 w-4" /></button><button type="button" onClick={() => setDeleting(statement)} aria-label={`Elimina ${row.label}`} className="flex h-7 w-7 items-center justify-center rounded-md text-red-500 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button></> : <button type="button" onClick={() => openRow(row)} className="flex h-7 items-center gap-1 rounded-md border border-gray-300 px-2 text-[11px] text-slate-600 hover:border-gray-400 hover:bg-slate-50"><Plus className="h-3.5 w-3.5" />Inserisci</button>}</div>
              </div>
            );
          })}
        </div>
