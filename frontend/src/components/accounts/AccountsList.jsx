...
export default function AccountsList({ accounts, onDelete, onEdit, onStatusChange }) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-lg border border-gray-300 bg-white">
      <div className="border-b border-gray-300 bg-slate-50 px-5 py-3">
      ...
      <div className="overflow-x-auto">
        <div style={{ minWidth: 900 }}>
          <div className="grid items-center gap-4 border-b border-gray-300 bg-slate-50 px-5 py-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400" style={{ gridTemplateColumns: accountGrid }}>
            ...
            {accounts.map((account) => (
              ...
              >
                ...
                <span className="flex items-center justify-end gap-1">
                  <button type="button" onClick={() => onEdit(account)} aria-label={`Modifica ${account.name}`} title="Modifica" className="flex h-7 w-7 items-center justify-center rounded-md text-slate-500 transition-colors hover:bg-orange-50 hover:text-orange-700">
