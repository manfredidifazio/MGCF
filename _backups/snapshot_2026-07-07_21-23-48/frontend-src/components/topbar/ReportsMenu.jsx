...
import { BanknotesIcon, DocumentChartBarIcon, PrinterIcon, ReceiptPercentIcon } from "@heroicons/react/24/outline";
import { useNavigate } from "react-router-dom";

const reports = [
  { label: "Resoconto contabile", path: "/accounting/reports", icon: BanknotesIcon },
  { label: "Resoconto fiscale", path: "/fiscal/reports", icon: ReceiptPercentIcon },
];
  ...
  return (
    <Menu as="div" className="relative shrink-0">
      ...
      <Menu.Items className="absolute right-0 top-full z-50 mt-1.5 w-52 overflow-hidden rounded-md border border-slate-200 bg-white py-1 focus:outline-none">
        <div className="flex items-center gap-2 px-3 py-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400"><DocumentChartBarIcon className="h-4 w-4" />Resoconti</div>
