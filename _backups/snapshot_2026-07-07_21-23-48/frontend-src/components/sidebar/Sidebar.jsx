...
function TreeLink({ to, children, className = "" }) {
  ...
  return <Link to={to} className={`${className} transition-colors hover:bg-amber-50 hover:text-orange-600 ${active ? "bg-amber-50 font-semibold text-orange-600" : "text-slate-600"}`}>{children}</Link>;
}
...
function SidebarLink({ item, minimal = false }) {
  ...
  return (
    <NavLink
      to={item.path}
      className={({ isActive }) => `flex items-center border-t border-slate-100 py-2.5 text-[10px] uppercase tracking-wide transition-colors hover:bg-amber-50 hover:text-orange-600 ${minimal ? "px-3" : "gap-2.5 px-1"} ${isActive ? "bg-amber-50 font-semibold text-orange-600" : "text-slate-600"}`}
    >
      {!minimal && <Icon className="h-4 w-4 shrink-0 text-slate-500" />}
...
function ToggleButton({ open, onToggle, label, compact = false }) {
  return (
    <button
      ...
      className={`flex shrink-0 items-center justify-center text-slate-500 transition-colors hover:text-amber-700 ${compact ? "h-7 w-7" : "h-8 w-8"}`}
    >
      {open ? <MinusIcon className={compact ? "h-3.5 w-3.5" : "h-4 w-4"} /> : <PlusIcon className={compact ? "h-3.5 w-3.5" : "h-4 w-4"} />}
    </button>
  );
}

function MacroSection({ label, open, onToggle, children }) {
  return (
    <section className="border-b border-slate-300 py-1.5">
      <div className="flex min-h-10 items-center justify-between">
        ...
        <button
          ...
          aria-expanded={open}
          aria-label={`${open ? "Nascondi" : "Mostra"} ${label}`}
          className="flex h-8 w-8 shrink-0 items-center justify-center text-slate-500 transition-colors hover:text-amber-700"
        >
  ...
  return (
    <div className="border-t border-slate-100">
      <div className="flex min-h-9 items-center">
        <TreeLink to={path} className="flex min-w-0 flex-1 items-center gap-2.5 px-1 py-2.5 text-[10px] font-medium uppercase tracking-wide">
    ...
    <div className="border-t border-slate-100">
      <div className="flex min-h-8 items-center">
        <button
          type="button"
          onClick={onToggle}
          className="flex min-w-0 flex-1 items-center gap-2 py-2 pl-1 text-[10px] font-medium uppercase tracking-wide transition-colors hover:bg-amber-50 hover:text-orange-600 text-slate-600"
        >
...
export default function Sidebar() {
  const [openMacros, setOpenMacros] = useState({ accounting: true, fiscal: true });
  ...
  function toggleMacro(macro) {
    setOpenMacros((current) => ({ ...current, [macro]: !current[macro] }));
  }
  ...
  return (
    <aside className="sticky top-14 h-[calc(100vh-56px)] w-64 shrink-0 overflow-y-auto border-r border-slate-200 bg-white px-3 pb-5">
      <MacroSection label="Gestione contabile" open={openMacros.accounting} onToggle={() => toggleMacro("accounting")}>
      ...
      <MacroSection label="Gestione fiscale" open={openMacros.fiscal} onToggle={() => toggleMacro("fiscal")}>
        {dynamicSections.map((section) => {
          const Icon = section.icon;
          return (
          <AccountingTreeGroup key={section.type} label={section.label} path={section.settingsPath} icon={Icon} open={openTrees[section.type]} onToggle={() => toggleTree(section.type)}>
        ...
        <SidebarLink item={{ label: "Resoconto fiscale", path: "/fiscal/reports", icon: ReceiptPercentIcon }} />
      </MacroSection>
    </aside>
  );
}
