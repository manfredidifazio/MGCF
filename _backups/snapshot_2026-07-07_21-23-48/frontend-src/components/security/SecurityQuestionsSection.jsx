...
export default function SecurityQuestionsSection() {
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-8">
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          {message.text && (
            <p className={`mt-6 text-sm font-medium ${message.error ? "text-red-600" : "text-emerald-600"}`}>
              {message.text}
            </p>
          )}
          ...
          <button
            type="submit"
            disabled={saving}
            className="mt-8 rounded-lg bg-slate-900 px-6 py-3 font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
          >
