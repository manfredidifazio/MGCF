...
export default function AccreditsPage() {
  ...
  const [listFilters, setListFilters] = useState({ dateFrom: "", dateTo: "", causeId: "", month: "" });
  ...
  const visibleAccredits = useMemo(() => scopedAccredits.filter((item) => {
    ...
    if (listFilters.month && String(item.movementDate).slice(5, 7) !== listFilters.month) return false;
    return true;
  }).sort((first, second) => String(second.movementDate).localeCompare(String(first.movementDate)) || second.id - first.id), [listFilters, scopedAccredits]);
  ...
  return (
    <div className="w-full h-full px-0 py-0">
      {message.text && <p className={`mb-0 rounded-none px-4 py-2 text-xs font-medium ${message.error ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>{message.text}</p>}

      {formOpen && (
        <form onSubmit={save} className="mb-2 rounded-lg border border-slate-200 bg-white p-8">
          ...
          <div className="mt-8 flex justify-end gap-3"><button type="button" onClick={closeForm} className="rounded-md border border-slate-300 px-6 py-3 font-semibold text-slate-700 hover:bg-slate-50">Annulla</button><button type="submit" disabled={saving || !selectedAccount || activeCauses.length === 0} className="rounded-md bg-amber-500 px-6 py-3 font-semibold text-white hover:bg-amber-600 disabled:opacity-60">{saving ? "Salvataggio..." : "Salva accredito"}</button></div>
        </form>
      )}

      <div className="mt-0 overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-4 py-2">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <BanknotesIcon className="h-5 w-5 shrink-0 text-amber-600" />
                <h1 className="text-xl font-semibold text-slate-900">ACCREDITI {selectedAccount?.name.toUpperCase() ?? "CONTO"} ANNO {globalFilters.year}</h1>
              </div>
              <p className="mt-1 text-sm text-slate-500">Gestisci gli accrediti, visualizza l'andamento e i totali per periodo.</p>
            </div>
            <div className="max-w-full overflow-x-auto pb-1 lg:ml-auto">
              <div className="ml-auto flex w-max flex-nowrap items-center gap-1.5">
                <span className="flex h-9 items-center rounded-md border border-slate-300 bg-white px-3 text-[11px] font-semibold uppercase tracking-wide text-slate-600">Anno {globalFilters.year}</span>
                <select value={globalFilters.month} onChange={(event) => {
                  const newParams = new URLSearchParams(searchParams);
                  if (event.target.value) {
                    newParams.set("month", event.target.value);
                  } else {
                    newParams.delete("month");
                  }
                  setSearchParams(newParams);
                }} className="flex h-9 items-center rounded-md border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-600 outline-none focus:border-amber-500">
                ...
                <button type="button" onClick={openNew} className="flex h-9 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-md bg-amber-500 px-4 text-sm font-semibold text-white transition-colors hover:bg-amber-600">
                  <PlusIcon className="h-4 w-4" /> Nuovo accredito {globalFilters.year}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="p-3">
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard label={statisticLabels.amount} value={currency.format(metrics.total)} icon={BanknotesIcon} />
            <MetricCard label={statisticLabels.count} value={String(metrics.count)} icon={CalculatorIcon} />
            <MetricCard label={statisticLabels.average} value={currency.format(metrics.average)} icon={CalculatorIcon} />
            <MetricCard
              ...
              detail={`Rispetto a ${metrics.previousPeriodLabel}: ${currency.format(metrics.previousMonth)}${metrics.percentage === null ? "" : ` · ${metrics.percentage >= 0 ? "+" : ""}${metrics.percentage.toFixed(1)}%`}`}
                icon={metrics.difference < 0 ? ExclamationTriangleIcon : metrics.difference > 0 ? ArrowTrendingUpIcon : CalculatorIcon}
                tone={metrics.difference < 0 ? "danger" : metrics.difference > 0 ? "success" : "neutral"}
              />
            </div>
        </div>
        ...
        <div className="border-t border-slate-200 px-4 py-2">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="mr-1 shrink-0 text-[10px] font-semibold uppercase tracking-wide text-slate-500">Per conto</h2>
            {metrics.byAccount.length ? metrics.byAccount.map((account) => (
              <div key={account.accountId} className="flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-xs">
                <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: account.accountColor || "#64748b" }} />
                <span className="font-semibold" style={{ color: account.accountColor || "#475569" }}>{account.accountName}</span>
                <span className="text-slate-400">{account.count}</span>
                <strong className="tabular-nums text-slate-800">{currency.format(account.total)}</strong>
              </div>
            )) : <p className="text-xs text-slate-400">Nessun accredito nel periodo</p>}
          </div>
        </div>

        <div className="border-t border-slate-200 px-4 py-2">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
            <div><h2 className="text-base font-semibold text-slate-900">Storico accrediti</h2><p className="mt-0.5 text-xs text-slate-500">{visibleAccredits.length} risultati nel periodo selezionato.</p></div>
            <div className="flex flex-wrap items-end gap-2">
              <FilterField label="Mese"><FilterSelect compact ariaLabel="Filtra storico per mese" value={listFilters.month} onChange={(event) => setListFilters((current) => ({ ...current, month: event.target.value }))}><option value="">Tutti</option>{months.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</FilterSelect></FilterField>
              ...
              {(listFilters.dateFrom || listFilters.dateTo || listFilters.causeId || listFilters.month) && <button type="button" onClick={() => setListFilters({ dateFrom: "", dateTo: "", causeId: "", month: "" })} className="h-8 rounded-md border border-slate-300 px-3 text-xs text-slate-600 hover:bg-slate-50">Azzera</button>}
            </div>
          </div>
        </div>
...
function MetricCard({ label, value, detail, icon: Icon, tone = "default" }) {
  ...
  return (
    <div role={danger ? "alert" : undefined} className={`rounded-lg border px-4 py-3 ${danger ? "border-red-300 bg-red-50" : "border-slate-200 bg-white"}`}>
      <div className="flex items-center justify-between"><p className={`text-[10px] font-medium uppercase tracking-wide ${danger ? "text-red-600" : "text-slate-400"}`}>{label}</p><Icon className={`h-4 w-4 ${danger ? "text-red-600" : success ? "text-emerald-600" : "text-amber-600"}`} /></div>
      <p className={`mt-1.5 text-xl font-semibold ${danger ? "text-red-700" : "text-slate-900"}`}>{value}</p>
      {detail && <p className={`mt-0.5 truncate text-[10px] ${danger ? "text-red-600" : success ? "text-emerald-600" : "text-slate-500"}`}>{detail}</p>}
    </div>
  );
}
...
function AccountAccreditsOverview({ account, accredits, causes, loading, message }) {
  ...
  return <div className="overflow-hidden rounded-lg border border-slate-200 bg-white p-3">
    ...
    <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard label="Importo complessivo" value={currency.format(overallTotal)} icon={BanknotesIcon} /><MetricCard label="Accrediti complessivi" value={String(accountAccredits.length)} icon={CalculatorIcon} /><MetricCard label="Media complessiva" value={currency.format(accountAccredits.length ? overallTotal / accountAccredits.length : 0)} icon={CalculatorIcon} /><MetricCard label="Anni registrati" value={String(recordedYears.size)} icon={ArrowTrendingUpIcon} /></div>
    ...
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3"><div><h2 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Impostazioni del conto</h2><p className="mt-0.5 text-xs text-slate-400">Configurazione del conto e causali disponibili.</p></div><div className="flex gap-2"><Link to="/settings/accounts" className="rounded-md border border-slate-300 px-3 py-2 text-xs text-slate-600 hover:bg-amber-50 hover:text-amber-700">Gestisci conto</Link><Link to="/settings/causes" className="rounded-md border border-slate-300 px-3 py-2 text-xs text-slate-600 hover:bg-amber-50 hover:text-amber-700">Gestisci causali</Link></div></div>
    ...
    <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard label={labels.amount} value={currency.format(total)} icon={BanknotesIcon} /><MetricCard label={labels.count} value={String(filtered.length)} icon={CalculatorIcon} /><MetricCard label={labels.average} value={currency.format(filtered.length ? total / filtered.length : 0)} icon={CalculatorIcon} /><MetricCard label={`Variazione ${monthKeyLabel(currentKey)}`} value={`${difference >= 0 ? "+" : ""}${currency.format(difference)}`} detail={`Rispetto a ${monthKeyLabel(previousKey)}: ${currency.format(previousTotal)}`} icon={difference < 0 ? ExclamationTriangleIcon : ArrowTrendingUpIcon} tone={difference < 0 ? "danger" : difference > 0 ? "success" : "neutral"} /></div>
    <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white"><div className="border-b border-slate-200 px-5 py-4"><h2 className="text-lg font-semibold text-slate-900">Storico del conto</h2><p className="mt-1 text-sm text-slate-500">{filtered.length} accrediti nella selezione corrente.</p></div>{loading ? <p className="px-5 py-6 text-sm text-slate-500">Caricamento...</p> : filtered.length ? <div className="overflow-x-auto"><div className="min-w-[620px]"><div className="grid grid-cols-[120px_minmax(180px,1fr)_130px_28px] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400"><span>Data</span><span>Causale</span><span className="text-right">Importo</span><span /></div><div className="divide-y divide-slate-100">{filtered.map((item) => <Link key={item.id} to={`/accounting/accredits?account=${account.id}&year=${String(item.movementDate).slice(0, 4)}`} className="grid grid-cols-[120px_minmax(180px,1fr)_130px_28px] items-center gap-4 px-5 py-2.5 text-[13px] hover:bg-amber-50"><span className="tabular-nums text-slate-500">{dateFormatter.format(new Date(`${String(item.movementDate).slice(0, 10)}T00:00:00`))}</span><span className="truncate text-slate-700">{item.causeName}</span><strong className="text-right tabular-nums text-slate-900">{currency.format(Number(item.amount))}</strong><ChevronRightIcon className="h-4 w-4 text-slate-400" /></Link>)}</div></div></div> : <p className="px-5 py-8 text-center text-sm text-slate-400">Nessun accredito corrisponde ai filtri</p>}</div>
  </div>;
}
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white p-3">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div><div className="flex items-center gap-3"><BanknotesIcon className="h-8 w-8 text-amber-500" /><h1 className="text-2xl font-semibold text-slate-900">Panoramica accrediti</h1></div><p className="mt-2 text-slate-500">Consulta tutti gli accrediti e restringi la panoramica per conto, periodo o causale.</p></div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Importo complessivo" value={currency.format(overallTotal)} icon={BanknotesIcon} />
        <MetricCard label="Accrediti complessivi" value={String(accredits.length)} icon={CalculatorIcon} />
        <MetricCard label="Media complessiva" value={currency.format(accredits.length ? overallTotal / accredits.length : 0)} icon={CalculatorIcon} />
        <MetricCard label="Anni registrati" value={String(overallYears.size)} icon={ArrowTrendingUpIcon} />
      </div>
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
        <AnnualOverviewChart data={annualData} />
        <AccountOverviewChart data={overallByAccount} />
      </div>
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3"><div><h2 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Impostazioni generali</h2><p className="mt-0.5 text-xs text-slate-400">Conti e causali utilizzati da tutti gli accrediti.</p></div><div className="flex gap-2"><Link to="/settings/accounts" className="rounded-md border border-slate-300 px-3 py-2 text-xs text-slate-600 hover:bg-amber-50 hover:text-amber-700">Gestisci conti</Link><Link to="/settings/causes" className="rounded-md border border-slate-300 px-3 py-2 text-xs text-slate-600 hover:bg-amber-50 hover:text-amber-700">Gestisci causali</Link></div></div>
