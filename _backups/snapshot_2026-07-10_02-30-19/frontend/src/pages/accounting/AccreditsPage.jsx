...
export default function AccreditsPage() {
  ...
  return (
    <div className="w-full h-full px-0 py-0">
      ...
      {formOpen && (
        <form onSubmit={save} className="mb-2 rounded-lg border border-slate-200 bg-white p-5">
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
            <label className="text-sm font-medium text-slate-700">Data accredito *<input type="date" required value={fields.movementDate} onChange={(event) => setFields((current) => ({ ...current, movementDate: event.target.value }))} className="mt-2 w-full rounded-md border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></label>
            <label className="text-sm font-medium text-slate-700">Importo accredito *<input type="number" required min="0.01" step="0.01" value={fields.amount} onChange={(event) => setFields((current) => ({ ...current, amount: event.target.value }))} placeholder="0,00" className="mt-2 w-full rounded-md border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></label>
            ...
            <label className="text-sm font-medium text-slate-700">Causale accredito *<select required value={fields.causeId} onChange={(event) => setFields((current) => ({ ...current, causeId: event.target.value }))} className="mt-2 w-full rounded-md border border-slate-300 bg-white px-4 py-3 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400"><option value="">Seleziona una causale</option>{activeCauses.map((cause) => <option key={cause.id} value={cause.id}>{cause.name}</option>)}</select><Link to="/settings/causes" className="mt-2 inline-block text-xs text-indigo-700 hover:text-indigo-900">Gestisci causali</Link></label>
            <label className="text-sm font-medium text-slate-700 md:col-span-2">Note<textarea rows="3" value={fields.notes} onChange={(event) => setFields((current) => ({ ...current, notes: event.target.value }))} placeholder="Nota facoltativa" className="mt-2 w-full rounded-md border border-slate-300 px-4 py-3 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></label>
          </div>
      ...
      <div className="mt-0 overflow-hidden rounded-lg border border-slate-200 bg-white">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-semibold text-white">ACCREDITI {selectedAccount?.name.toUpperCase() ?? "CONTO"} ANNO {globalFilters.year}</h1>
              </div>
              <p className="mt-1 text-sm text-indigo-100">Gestisci gli accrediti, visualizza l'andamento e i totali per periodo.</p>
            </div>
            <div className="max-w-full overflow-x-auto pb-1 lg:ml-auto">
              <div className="ml-auto flex w-max flex-nowrap items-center gap-1.5">
                <span className="flex h-9 items-center rounded-md border border-indigo-400 bg-indigo-50 px-3 text-[11px] font-semibold uppercase tracking-wide text-indigo-600">Anno {globalFilters.year}</span>
                <div className="relative shrink-0">
                  ...
                  }} className="appearance-none h-9 rounded-md border border-indigo-300 bg-indigo-50 px-3 pr-7 text-xs font-semibold text-indigo-700 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400">
                    <option value="">Tutti i mesi</option>
                    {months.map(([value, label]) => (
                      <option key={value} value={value}>{label}</option>
                    ))}
                  </select>
                  <ChevronDown className="pointer-events-none absolute right-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-indigo-500" />
                </div>
                <button type="button" onClick={openNew} className="flex h-9 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-md border border-white/35 bg-white px-4 text-sm font-semibold text-indigo-700 shadow-sm transition-colors hover:bg-indigo-50">
                  <Plus className="h-4 w-4" /> Nuovo accredito
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="p-3">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Stat label={statisticLabels.count} value={String(metrics.count)} icon={Calculator} tone="count" />
            <Stat label={statisticLabels.average} value={currency.format(metrics.average)} icon={Calculator} tone="average" />
            <Stat
        ...
        <div className="border-t border-slate-200 px-4 py-2">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <FilterField label="Dal"><input type="date" value={listFilters.dateFrom} onChange={(event) => setListFilters((current) => ({ ...current, dateFrom: event.target.value }))} className="h-8 rounded-md border border-slate-300 px-2 text-xs outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></FilterField>
              <FilterField label="Al"><input type="date" value={listFilters.dateTo} onChange={(event) => setListFilters((current) => ({ ...current, dateTo: event.target.value }))} className="h-8 rounded-md border border-slate-300 px-2 text-xs outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400" /></FilterField>
...
function Stat({ label, value, detail, icon: Icon, tone = "default", danger = false }) {
  ...
  return (
    <div role={danger || tone === "danger" ? "alert" : undefined} className={`rounded-lg border px-4 py-3 shadow-sm ${color}`}>
      ...
      <strong className="mt-1.5 block min-h-8 text-2xl font-bold">{value || "\u00A0"}</strong>
      {detail && <span className={`mt-0.5 block text-[10px] ${labelColor}`}>{detail}</span>}
    </div>
  );
}
  ...
  return (
    <div className="relative shrink-0">
      <select
        ...
        value={value}
        onChange={onChange}
        className={`appearance-none truncate rounded-md border border-indigo-300 bg-indigo-50 pl-2.5 pr-7 text-indigo-700 outline-none transition-colors hover:border-indigo-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-400 ${widthClass} ${compact ? "h-8 text-xs" : "h-9 text-xs"}`}
      >
        {children}
      </select>
      <ChevronDown className="pointer-events-none absolute right-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-indigo-500" />
