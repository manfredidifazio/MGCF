    ...
    <PageMask title="Amministrazione utenti" description="Gestisci gli utenti registrati e abilita solo quelli approvati.">
      ...
      <div className="mb-3 flex gap-2">
        <input
          ...
          className="h-9 rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-amber-500"
        />
        <button type="button" onClick={() => loadUsers()} className="rounded-md border border-slate-300 px-4 text-sm">
          Cerca
        </button>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <div style={{ minWidth: 1120 }}>
          <div
            className="grid gap-4 border-b border-slate-200 bg-slate-50 px-5 py-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400"
      ...
      {pendingEdit && (
        <Modal title="Modificare utente?">
          ...
          <input
            ...
            className="mt-3 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-amber-500"
          />
          <ModalActions
            cancelLabel="Annulla"
...
function ActionButton({ children, onClick, tone = "slate" }) {
  const tones = {
    slate: "border-slate-300 text-slate-700 hover:border-amber-300 hover:bg-amber-50 hover:text-amber-700",
