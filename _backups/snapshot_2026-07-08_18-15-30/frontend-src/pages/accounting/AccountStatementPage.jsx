    ...
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <div>
        <div className="flex items-center gap-2">
          <BarChart className="h-5 w-5 shrink-0 text-amber-600" />
          <h1 className="text-xl font-semibold text-slate-900">ESTRATTI CONTO {account.name.toUpperCase()} ANNO {year}</h1>
        </div>
        <p className="mt-1 text-sm text-slate-500">Gestisci i saldi mensili, visualizza le variazioni e i totali annuali.</p>
      </div>
      </div>
      </div>
...
function Stat({ label, value, icon: Icon, danger = false, detail, tone = "default" }) {
  const toneClasses = {
    ...
    accountBalance: "border-orange-200 bg-orange-50 text-orange-800",
  ...
  return (
    <div role={danger ? "alert" : undefined} className={`rounded-lg border px-4 py-3 ${color}`}>
      ...
      <strong className="mt-1.5 block min-h-8 text-xl">{value || "\u00A0"}</strong>
      ...
      <div className="divide-y divide-slate-200 rounded-lg border border-slate-200 bg-slate-50/60">
        <div className="px-3 py-2">
          <div className="flex items-center justify-between gap-3">
            ...
            <TrendingUp className={`h-4 w-4 ${bestDanger ? "text-red-600" : "text-emerald-600"}`} />
          </div>
