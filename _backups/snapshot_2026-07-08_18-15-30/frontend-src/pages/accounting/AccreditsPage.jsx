...
export default function AccreditsPage() {
  ...
  useEffect(() => {
    if (formOpen) scrollToElement(formRef.current);
  }, [formOpen]);
  ...
  return (
    <div className="w-full h-full px-0 py-0">
      ...
      {formOpen && (
        <form ref={formRef} onSubmit={save} className="mb-2 rounded-lg border border-slate-200 bg-white p-8">
      ...
      <div className="mt-0 overflow-hidden rounded-lg border border-slate-200 bg-white">
            ...
            <div className="max-w-full overflow-x-auto pb-1 lg:ml-auto">
              <div className="ml-auto flex w-max flex-nowrap items-center gap-1.5">
                ...
                <button type="button" onClick={openNew} className="flex h-9 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-md bg-amber-500 px-4 text-sm font-semibold text-white transition-colors hover:bg-amber-600">
                  <Plus className="h-4 w-4" /> Nuovo accredito {globalFilters.year}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="p-3">
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
            <MetricCard label={statisticLabels.amount} value={currency.format(metrics.total)} icon={Banknote} />
            <MetricCard label={statisticLabels.count} value={String(metrics.count)} icon={Calculator} />
            <MetricCard label={statisticLabels.average} value={currency.format(metrics.average)} icon={Calculator} />
            <MetricCard
...
function AccountAccreditsOverview({ account, accredits, causes, loading, message }) {
  ...
  return <div className="overflow-hidden rounded-lg border border-slate-200 bg-white p-3">
    ...
    <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard label="Importo complessivo" value={currency.format(overallTotal)} icon={Banknote} /><MetricCard label="Accrediti complessivi" value={String(accountAccredits.length)} icon={Calculator} /><MetricCard label="Media complessiva" value={currency.format(accountAccredits.length ? overallTotal / accountAccredits.length : 0)} icon={Calculator} /><MetricCard label="Anni registrati" value={String(recordedYears.size)} icon={TrendingUp} /></div>
    ...
    <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard label={labels.amount} value={currency.format(total)} icon={Banknote} /><MetricCard label={labels.count} value={String(filtered.length)} icon={Calculator} /><MetricCard label={labels.average} value={currency.format(filtered.length ? total / filtered.length : 0)} icon={Calculator} /><MetricCard label={`Variazione ${monthKeyLabel(currentKey)}`} value={`${difference >= 0 ? "+" : ""}${currency.format(difference)}`} detail={`Rispetto a ${monthKeyLabel(previousKey)}: ${currency.format(previousTotal)}`} icon={difference < 0 ? AlertTriangle : TrendingUp} tone={difference < 0 ? "danger" : difference > 0 ? "success" : "neutral"} /></div>
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white p-3">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div><div className="flex items-center gap-3"><Banknote className="h-8 w-8 text-amber-500" /><h1 className="text-2xl font-semibold text-slate-900">Panoramica accrediti</h1></div><p className="mt-2 text-slate-500">Consulta tutti gli accrediti e restringi la panoramica per conto, periodo o causale.</p></div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Importo complessivo" value={currency.format(overallTotal)} icon={Banknote} />
        <MetricCard label="Accrediti complessivi" value={String(accredits.length)} icon={Calculator} />
        <MetricCard label="Media complessiva" value={currency.format(accredits.length ? overallTotal / accredits.length : 0)} icon={Calculator} />
        <MetricCard label="Anni registrati" value={String(overallYears.size)} icon={TrendingUp} />
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label={labels.amount} value={currency.format(total)} icon={Banknote} />
        <MetricCard label={labels.count} value={String(filtered.length)} icon={Calculator} />
        <MetricCard label={labels.average} value={currency.format(filtered.length ? total / filtered.length : 0)} icon={Calculator} />
        <MetricCard label={`Variazione ${monthKeyLabel(currentKey)}`} value={`${difference >= 0 ? "+" : ""}${currency.format(difference)}`} detail={`Rispetto a ${monthKeyLabel(previousKey)}: ${currency.format(previousTotal)}`} icon={difference < 0 ? AlertTriangle : TrendingUp} tone={difference < 0 ? "danger" : difference > 0 ? "success" : "neutral"} />
      </div>
