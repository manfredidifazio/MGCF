...
export async function reorderManagedItemsHandler(req, res) {
  if (!validType(req, res)) return;
  if (!Array.isArray(req.body.items)) {
    return res.status(400).json({ success: false, message: "Lista di elementi non valida." });
  }
  try {
    ...
    const updated = await reorderManagedItems(req.user.id, req.params.type, items);
    return res.json({ success: true, items: updated });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: "Impossibile aggiornare l'ordine." });
  }
}
